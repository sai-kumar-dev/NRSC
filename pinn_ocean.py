...
def calculate_comprehensive_metrics(y_true, y_pred, threshold_percentile=50):
    ...
    # For classification metrics, we'll create binary classes based on flux direction
    # Positive flux = 1, Negative flux = 0
...
def main():
    ...
    try:
        ...
        config = {
            ...
            'lambda_physics': 0.1,
            'train_split': 0.8,
            'test_split': 0.2  # Changed to 80-20 split
        }
...
def create_performance_summary():
    ...
    if os.path.exists(metrics_path):
        ...
        summary_data = {
            ...
            'Sensible Heat Flux': [
                f"{metrics['sensible_heat_flux']['RMSE']:.4f}",
                f"{metrics['sensible_heat_flux']['MSE']:.4f}",
                f"{metrics['sensible_heat_flux']['MAE']:.4f}",
                f"{metrics['sensible_heat_flux']['R2']:.4f}",
                f"{metrics['sensible_heat_flux']['Accuracy']:.4f}",
                f"{metrics['sensible_heat_flux']['Precision']:.4f}",
                f"{metrics['sensible_heat_flux']['Recall']:.4f}",
                f"{metrics['sensible_heat_flux']['F1_Score']:.4f}"
            ],
            'Latent Heat Flux': [
                f"{metrics['latent_heat_flux']['RMSE']:.4f}",
                f"{metrics['latent_heat_flux']['MSE']:.4f}",
                f"{metrics['latent_heat_flux']['MAE']:.4f}",
                f"{metrics['latent_heat_flux']['R2']:.4f}",
                f"{metrics['latent_heat_flux']['Accuracy']:.4f}",
                f"{metrics['latent_heat_flux']['Precision']:.4f}",
                f"{metrics['latent_heat_flux']['Recall']:.4f}",
                f"{metrics['latent_heat_flux']['F1_Score']:.4f}"
            ]
        }
